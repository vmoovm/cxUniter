(function () {
	var IScroll = (function (window, document, Math) {
		...
		var deg = 25  // 默认25角度  经配置可变
		var itemCount = 7 // 默认个数为7 经配置可变
		var utils = (function () {
		    var me = {};
		...
		/* REPLACE END: _move */
		    ...
		    refresh: function () {
		        ...
		        if (this.scroller.children[0]) {
		        	if (itemCount === 5) {
		        		this.scrollerHeight = deg * (this.scroller.children.length - 2);
		        		this.wrapperHeight  = deg * 5;
		        	} else if (itemCount === 7) {
		        		this.wrapperHeight  = deg * 5;
		        		this.scrollerHeight = deg * (this.scroller.children.length - 4);
		        	}
		        } else {
		        	return
		        }
		    ...
		    scrollToElement: function (el, time, offsetX, offsetY, easing) {
		        el = (el - 1) * deg * -1
		        this.scrollTo(0, el, time, easing);
		    },
		    ...
		    _translate: function (x, y) {
		        if ( this.options.useTransform ) {
					this.scrollerStyle[utils.style.transform] = 'perspective(1000px) rotateY(0deg) rotateX(' + -y + 'deg)';
		        } else {
		            x = Math.round(x);
		            y = Math.round(y);
	...
	function Layer (html, opts) {
		...
		close_el.className = 'closeTarget';
		this.el = el;
		this.layer_el = layer_el;
		this.close_el = close_el;
		this.init();
	}
	...
	Layer.prototype = {
		init: function () {
			...
			this.el.insertBefore(this.close_el, this.layer_el);
		...
		bindEvent: function () {
			...
			var closeTargetDom = this.el.querySelectorAll('.closeTarget');
			var self = this;
			...
			for (var i = 0, len = closeTargetDom.length; i < len; i++) {
				closeTargetDom[i].addEventListener('click', function (e) {
					self.close();
				});
			}
		},
	...
	function IosSelect (level, data, options) {
		...
		this.options.itemShowCount = [5, 7].indexOf(options.itemShowCount) !== -1 ? options.itemShowCount : 7; 
		...
		if (this.options.deg) {
			IScroll.prototype._deg(this.options.deg)
		}
		if (this.options.itemShowCount) {
			IScroll.prototype._itemCount(this.options.itemShowCount)
		}
		this.setBase();
		this.init();
	};
	...
	IosSelect.prototype = {
		...
		initLayer: function () {
			var self = this;
			var stringSpace = '' // 仅空白字符串，值不会变，便于字符串拼接
			var displayStyle = this.options.unitItem.length ? '' : 'display: none;' // 列单位数组为空时隐藏
			var all_html = [
				...
				'</header>',
				'<div style="height: ' + this.options.unitHeight + this.options.cssUnit + '; line-height: ' + this.options.unitHeight + this.options.cssUnit + ';' + displayStyle + ';" class="iosselect-unit">',
					'<div class="iosselect-unitCol one-level-unitheight">' + (this.options.unitItem.length ? this.options.unitItem[0] : stringSpace) + '</div>',
					'<div class="iosselect-unitCol two-level-unitheight">' + (this.options.unitItem.length ? this.options.unitItem[1] : stringSpace) + '</div>',
					'<div class="iosselect-unitCol three-level-unitheight">' + (this.options.unitItem.length ? this.options.unitItem[2] : stringSpace) + '</div>',
					'<div class="iosselect-unitCol four-level-unitheight">' + (this.options.unitItem.length ? this.options.unitItem[3] : stringSpace) + '</div>',
					'<div class="iosselect-unitCol five-level-unitheight">' + (this.options.unitItem.length ? this.options.unitItem[4] : stringSpace) + '</div>',
				'</div>',
			...
			this.iosSelectLayer.el.querySelector('.layer').style.height = this.options.itemHeight * this.options.itemShowCount + this.options.headerHeight + this.options.unitHeight + this.options.cssUnit;
			...
			this.scrollOne.on('scroll', function () {
				var pa = Math.abs(this.y / self.options.deg)
				var plast = 1;
			...
			this.scrollOne.on('scrollEnd', function () {
				var pa = Math.abs(this.y / self.options.deg)
					...
					if (Math.ceil(pa) === Math.round(pa)) {
						to = Math.ceil(pa) * self.options.deg
						plast = Math.ceil(pa) + 1;
					} else {
						to = Math.floor(pa) * self.options.deg
			...
			this.closeTargetDom = this.iosSelectLayer.el.querySelector('.closeTarget');
			this.closeTargetDom.addEventListener('click', function(e) {
				if (document.body.classList.contains('ios-select-body-class')) {
					document.body.classList.remove('ios-select-body-class');
				}
				window.scrollTo(0, self.offsetTop);
		...
		renderOneLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, oneLevelData) {
			...
			oneLevelData.forEach(function(v, i, o) {
				if (v.id == oneLevelId) {
					oneHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';" ' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v.value + '</li>';
					plast = i + 1;
				} else {
					oneHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v.value + '</li>';
				}
			}.bind(this));
			oneHtml += this.getWhiteItem();
			...
			// this.scrollOne.scrollToElement('li:nth-child(' + plast + ')', 0); console.log(plast)
			this.scrollOne.scrollToElement(plast, 0);
			if (this.level >= 2) {
		...
		renderTwoLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, twoLevelData) {
			...
			twoLevelData.forEach(function(v, i, o) {
				if (v.id == twoLevelId) {
					twoHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v.value + '</li>';
					plast = i + 1;
				} else {
					twoHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v.value + '</li>';
				}
			}.bind(this));
			twoHtml += this.getWhiteItem();
			this.twoLevelUlContainDom.innerHTML = twoHtml;
			this.scrollTwo.refresh();
			this.scrollTwo.scrollToElement(plast, 0);
			if (this.level >= 3) {
	    ...
	    renderThreeLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, threeLevelData) {
			...
			threeLevelData.forEach(function(v, i, o) {
				if (v.id == threeLevelId) {
					threeHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v.value + '</li>';
					plast = i + 1;
				} else {
					threeHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v.value + '</li>';
				}
			}.bind(this));
			threeHtml += this.getWhiteItem();
			this.threeLevelUlContainDom.innerHTML = threeHtml;
			this.scrollThree.refresh();
			this.scrollThree.scrollToElement(plast, 0);
	    ...
	    renderFourLevel: function (oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, fourLevelData) {
			...
			fourLevelData.forEach(function(v, i, o) {
				if (v.id == fourLevelId) {
					fourHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v.value + '</li>';
					plast = i + 1;
				} else {
					fourHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v.value + '</li>';
				}
			}.bind(this));
			fourHtml += this.getWhiteItem();
			this.fourLevelUlContainDom.innerHTML = fourHtml;
			this.scrollFour.refresh();
			this.scrollFour.scrollToElement(plast, 0);
	    ...
	    renderFiveLevel: function (oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, fiveLevelData) {
			...
			fiveLevelData.forEach(function(v, i, o) {
				if (v.id == fiveLevelId) {
					fiveHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v.value + '</li>';
					plast = i + 1;
				} else {
					fiveHtml += '<li style="transform: translateZ(' + this.options.translateZ + this.options.cssUnit + ') rotateX(-' + (i * self.options.deg) + 'deg); height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v.value + '</li>';
				}
			}.bind(this));
			fiveHtml += this.getWhiteItem();
			this.fiveLevelUlContainDom.innerHTML = fiveHtml;
			this.scrollFive.refresh();
			this.scrollFive.scrollToElement(plast, 0);
	    ...
	    changeClassName: function (levelContainDom, plast) {
	    	var pdom;
	    	if (this.options.itemShowCount === 5 && parseInt(plast) >= 0) {
	    	...
	    	} else if (this.options.itemShowCount === 7 && parseInt(plast) >= 0) {
				...
				levelContainDom.querySelector('li:nth-child(' + (plast + 7) + ')').classList.add('side2');
	    	}
	    ...
	    setBase: function () {
			if (this.options.cssUnit === 'rem') {
				...
				var dltStyle = window.getComputedStyle(dltDom, null); // 获得所有dom所支持的样式对象
